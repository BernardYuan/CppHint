...
static cl::list<string> libPaths{
    ...
    cl::value_desc{"directory"}, cl::cat{callProfilerCategory}};

static cl::list<string> libraries{
    "l", cl::Prefix, cl::desc{"Specify libraries to link to"},
    cl::value_desc{"library prefix"}, cl::cat{callProfilerCategory}};
...
static void link(StringRef objectFile, StringRef outputFile) {
  ...
  string err;
  if (-1 ==
      ExecuteAndWait(clang.get(), &charArgs[0], nullptr, nullptr, 0, 0, &err)) {
