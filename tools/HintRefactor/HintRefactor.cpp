...
class HintRefactorHandler : public MatchFinder::MatchCallback {
 public:
  HintRefactorHandler(map<string, Replacements> *replacementMap,
                      string replaceString)
      : replacementMap(replacementMap), replacementString(replaceString) {}
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    if (const CallExpr *E = Result.Nodes.getNodeAs<CallExpr>("breakpoint")) {
      ...
      if (startLocation.isValid() && endLocation.isValid()) {
        // Compute the offset.
        unsigned offset =
            E->getLocEnd().getRawEncoding() - E->getLocStart().getRawEncoding();

        Replacement hintReplacement(
            *(Result.SourceManager), E->getLocStart(), offset + 2,
...
static cl::opt<string> ReplacementStatement(
    "s", cl::desc("The statement to replace CppHint."), cl::Required,
...
int main(int argc, const char **argv) {
  ...
  HintRefactorHandler hintRefactorHandler(&(Tool.getReplacements()),
  ...
  if (int res =
          Tool.runAndSave(newFrontendActionFactory(&breakPointFinder).get())) {
    return res;
  }
  return 0;
}
